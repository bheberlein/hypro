...
def interpolate_bad_detectors(rdn_image, bad_detectors_list, wavelengths, step=1):
    ''' Perform in-place modification of raw radiance cube, interpolating
         values from faulty detectors using neighboring wavelengths as
         endpoints.
    '''
    for band, column in bad_detectors_list:
        center_wavelength = wavelengths[band]
        endpoint_wavelengths = wavelengths[[band-step, band+step]] # Shape (2,)
        endpoint_radiances = rdn_image[:,[band-step, band+step],column] # Shape (n_scanlines, 2)
        interpolated = np.array([np.interp(center_wavelength, endpoint_wavelengths, endpoint_rdn) for  endpoint_rdn in endpoint_radiances])
        rdn_image[:,band,column] = interpolated

def do_interpolate_bad_detectors_Hyspex(sensor_dict):
    ''' Do radiance interpolation over bad detectors. '''
    ...
    rdn_image = np.memmap(sensor_dict['raw_rdn_image_file'],
                          ...
                          mode='r+', dtype='int16')
    try:
        # If `False`, DON'T do the interpolation
        if sensor_dict['bad_detectors'] is False: return
        # If `None`, identify bad detectors 'on-the-fly'
        elif sensor_dict['bad_detectors'] is None: raise ValueError
        # Otherwise, use the bad detectors list given
        else: bad_detectors = np.array(sensor_dict['bad_detectors'], dtype=np.int)
    
    except (KeyError, ValueError):
        logger.info('Identify bad detectors from the raw radiance cube.')
        # Identify bad detectors 'on-the-fly'
        bad_detectors = identify_bad_detectors(rdn_image, hdr, step=1, threshold=None)
        logger.info(f'{len(bad_detectors)} bad detectors found: {str(bad_detectors)}')
                
    # Interpolate over bad detectors
    logger.info('Do interpolation.')
    wavelengths = np.array(hdr['wavelength'], dtype=rdn_image.dtype)
    interpolate_bad_detectors(rdn_image, bad_detectors, wavelengths, step=1)
