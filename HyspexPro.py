...
(3) Radiometric correction focuses on reducing radiometric distortions mainly caused by
    ...
    Gao [1] was adapted to detect sensor smile effects. Then, raw radiance spectra were resampled to common center wavelengths
    ...
    technique proposed by Kaufman [2]. The estimation of visibility was based on the Dense Dark Vegetation (DDV) method
    proposed by Kaufman [3]. All radiometric correction steps require an atmosphere lookup table which consists of
    the at-senor radiance simulated under different atmospheric and sun-target-view geometric conditions.
    An open-source radiative transfer model, LibRadTran, was used for this purpose.
    For more details about LibRadTran, please refer to http://www.libradtran.org/doku.php.
...
def HyspexPro(config_file):
    ...
    for flight_index in flight_indices:
        ...
        """
        sensor_dict = list(flight_dict['sensors'].values())[0]
        raw_imugps = np.loadtxt(sensor_dict['raw_imugps_file'])
        if raw_imugps.shape[0]<500:
            print("%s: Not processed!" %flight_index)
            continue
        del sensor_dict
        """
        ...
        for sensor_index, sensor_dict in flight_dict['sensors'].items():
            ...
            do_interpolate_bad_detectors_Hyspex(sensor_dict)
