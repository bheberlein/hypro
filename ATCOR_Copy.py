...
def main(in_dir, atcor_dir, idl_file):
    cmd_file = os.path.join(in_dir, '%s_atcor_cmd.txt' %(os.path.basename(in_dir)))
    fid = open(cmd_file, 'w')
    ...
    for flight_dir in flight_dirs:
        ...
        new_sca_image_file = search_file(merge_dir, '*SCA_New')
        dem_image_file = search_file(merge_dir, '*DEM')
        if (rdn_image_file is None) or (new_sca_image_file is None) or (dem_image_file is None):
            continue
        if (not os.path.exists(rdn_image_file+'.hdr')) or (not os.path.exists(new_sca_image_file+'.hdr')) or (not os.path.exists(dem_image_file+'.hdr')):
            continue
        ...
        new_sca_header = read_envi_header(os.path.splitext(new_sca_image_file)[0]+'.hdr')
        ...
        params['Scale factor reflectance'] = ' 100.0    '

        # 3: pixel size
        params['Pixel size [m]'] = '    %s    ' %(rdn_header['map info'][5])
        
        # 4: sensor name
        params['sensor name'] = 'hyspex, hyspex,'

        # 5: gain setting
        params['gainset'] = '    1.0        '

        # 6: calibration file
        cali_fn = glob.glob(os.path.join(atcor_dir, 'sensor', 'hyspex', '*.cal'))
        params['cali_fn'] = cali_fn[0]
        
        # 7: scan angle file
        params['sca_fn'] = new_sca_image_file
        ...
        # 14: atm filename
        altitude = float(new_sca_header['GPS long-lat-alt'].split(',')[2])
        ...
        #params['refl_atm_fn'] = os.path.join(atcor_dir, 'atm_lib', 'hyspex', basename)
        ...
        avg_elev = float(new_sca_header['DEM height[m]'])
        ...
        params['Solar zenith, azimuth [degree]'] = '    %.1f    %.1f    ' %(float(new_sca_header['sun zenith']), float(new_sca_header['sun azimuth']))

        # 20: Flight altitude [km], heading[deg]
        altitude = altitude/1000.0 #meter to km
        heading = float(new_sca_header['heading[deg]'])
        ...
        """
        params['npref, iwaterwv, ihaze, iwat_shd, ksolflux, ishadow, icl_shadow'] = '    1 1 0 0 0 0 0     '
        ...
        params['bands for 820 nm water vapor retrieval'] = ' 122 122 130 135 137 138    '

        # bands for 1130 nm water vapor retrieval
        params['bands for 1130 nm water vapor retrieval'] = '  0 0 0 0 0 0     '
...
atcor_dir = 'E:/rese/atcor_4'
idl_file = 'E:/rese/IDL84/bin/bin.x86_64/idlrt.exe'
in_dir = 'Z:/townsenduser-rw/hyspex_processed/Cranberry_Creek'
for site_dir in glob.glob(os.path.join(in_dir, '*')):
    main(site_dir, atcor_dir, idl_file)
