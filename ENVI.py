...
envi_to_numpy_dtype_map = {1:np.uint8,
                           2:np.int16,
                           3:np.int32,
                           4:np.float32,
                           5:np.float64,
...
def numpy_dtype_from_envi(hdr):
    """ Get data type from ENVI header.
    Arguments:
        hdr: dict
            Dictionary containing ENVI header entries.
    Returns:
        _: np.dtype
            Data type as an numpy.dtype object.
    """
    
    return envi_to_numpy_dtype_map[hdr['data type']]